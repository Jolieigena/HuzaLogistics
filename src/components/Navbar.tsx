...
export function Navbar() {
  ...
  return (
    <header className="w-full bg-white border-b border-slate-100 py-4 px-6 sm:px-10 flex items-center justify-between sticky top-0 z-40 backdrop-blur-md bg-white/90">
      {/* Brand Logo */}
      <Link href="/" className="flex items-center gap-2 text-slate-900 font-extrabold text-xl sm:text-2xl tracking-tight group">
        <HuzaLogo className="w-6 h-6 sm:w-7 sm:h-7 text-emerald-600 group-hover:text-emerald-700 transition-colors" />
        <span className="tracking-tight">HuzaLogistics</span>
      </Link>
      ...
      <div className="flex items-center gap-3">
        ...
        <Link
          href="/licensing"
          className="hidden sm:inline-flex items-center gap-1 bg-white hover:bg-slate-50 border border-slate-300 text-slate-800 font-medium text-sm px-5 py-2.5 rounded-full transition-all duration-200"
        >
